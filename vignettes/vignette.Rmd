...
exists a unique identifier that unambiguously identifies records, in which case the \texttt{merge()} function in \R\ 
...
In this paper, we introduce a new \R\ package, \fastLink\ (@enam:fifi:imai:19) for conducting data merges. 
...
$\bm{\tau} = \{ \tau_0, \tau_1, \ldots, \tau_{L_{k - 1}} \}$ the set
...
$\bm{\tau} = \{ 0.88, 0.94\}$ to construct $\gamma_k$ for three
agreement levels.
...
\begin{itemize}
-  The number of unique values observed in each linkage field is
  often less than the number of observations in each dataset. For
  example, consider a variable such as first name. Naively, one may
  compare the first name of each observation in dataset $\mathcal{A}$
  with that of every observation in $\mathcal{B}$. In practice,
  however, we can reduce the number of comparisons by considering only
  unique first name that appears in each data set.  The same trick can
  be used for all linkage fields by focusing on the comparison of the
  unique values of each variable.

-  For each comparison between two unique first names
  ($name_{1, \mathcal{A}}$ and $name_{1, \mathcal{B}}$), for example,
  we only keep the indices of the original datasets and store them
  using what is often referred as a reverse data structure in the
  literature [@pchri:12]. In such an arrangement, a pair of names
  ($name_{1, \mathcal{A}}$, $name_{1, \mathcal{B}}$) becomes a key
  with two lists, one containing the indices from dataset
  $\mathcal{A}$ that have a first name equal to
  $name_{1, \mathcal{A}}$, and another list that does the same for
  $name_{1, \mathcal{B}}$ in dataset $\mathcal{B}$.

-  Comparisons involving a missing value need not be made. Instead,
  we only need to store the indices of the observations in
  $\mathcal{A}$ and $\mathcal{B}$ that contain missing information for
  field $k$.

-  Since the agreement levels are mutually exclusive, we use the
  lowest agreement level as the base category. Once a set of threshold
  values has been defined, then a pair of names can only be
  categorized in one of the $L_k$ agreement levels. The indices for
  the the pairs of values that can be categorized as disagreements (or
  nearly disagreements) do not need to be stored.  For most variables,
  disagreement is the category that encompasses the largest number of
  pairs.  Thus, our reverse data structure lists become quite sparse.
  This sparsity can be exploited by the use of sparse matrix, yielding
  a substantially memory efficient implementation.
\end{itemize}
...
summarizing output from these functions (e.g., \texttt{plot()} and \texttt{confusion()}).
Figure~\@ref(fg:schematic) illustrates the core structure of the \fastLink\ package.

%% ------------------------------
%% Begin Tikz figure of flowchart
%% ------------------------------
...
\end{figure}
%% ----------------------------
%% End Tikz figure of flowchart
%% ----------------------------

The core of the algorithm can be run using the \texttt{fastLink()} wrapper, which we will describe in
...
The \texttt{fastLink()} function takes two data sets and a simple description of the merging fields, and returns
the results of the data merge which can then be passed to the \texttt{plot()} and \texttt{confusion()} functions for inspection and summarization. We
illustrate the use of the function using a small-scale example, which is included in the \fastLink\ package.
The two data sets, \texttt{dfA} and \texttt{dfB}, are samples from the California voter file which are then shuffled within-field
...
Here, we run a simple match using \texttt{fastLink()} to merge \texttt{dfA} and \texttt{dfB}:
...
\begin{itemize}
-  \texttt{matches}: A matrix with two columns and a row for each matched pair above the specified threshold. The first column, \texttt{inds.a}, gives the row numbers of the successfully matched observations in datset A, while the second column \texttt{inds.b} gives the row numbers of successfully matched observations in dataset B. The user can recover subsetted data frames by specifying \texttt{return.df = TRUE} in \texttt{fastLink()}, or by using the function \texttt{getMatches()}.
-  \texttt{EM}: Parameter estimates from the Expectation-Maximization algorithm, which are used to calculate the confusion table and visualize the quality of different matching patterns.
-  \texttt{patterns}: The matching patterns for each match variable, sorted to correspond to the pairs in \texttt{matches}. Here, 2 indicates an exact match, 0 indicates a non-match, 1 indicates a partial match, and \texttt{NA} indicates a case where one or both of the observations had a missing value.
-  \texttt{posterior}: The posterior matching probability for each matched pair, sorted to correspond to the pairs in \texttt{matches}.
-  \texttt{nobs.a}, \texttt{nobs.b}: The number of observations in dataset A and dataset B, respectively.
\end{itemize}

As mentioned above, the user can recover a data frame of the successfully matched observations using the \texttt{getMatches()} function. The resulting data frame contains the union of all unique column names between the two merged data frames, as well as the match patterns and the posterior matching probability, as shown below:
...
\end{eqnarray*} The function returns a number of other summary metrics about the quality of the match, including sensitivity, specificity, the F1 score, and the percent of observations correctly classified as matches and non-matches.

Finally, we can visualize the matching patterns using the \texttt{plot()} function, as follows:
...
The argument \texttt{stringdist.match} contains a vector of variables that should be matched using a string-distance metric, which is specified in \texttt{stringdist.method}. Here, we've specified that \texttt{fastLink()} should use the Jaro-Winkler metric (\texttt{jw}), although other valid metrics include the Jaro metric (\texttt{jaro}) or the Levenshtein distance (\texttt{lv}). Finally, \texttt{cut.a} specifies that if the string similarity measure between pairs of observations is above 0.94, the pair should be coded as a match. Default settings are \texttt{stringdist.method = "jw"} and \texttt{cut.a = 0.94} (following the recommendations of @Winkler1990).

Specifying the variables to be compared using numeric matching is similar. If, for example, we believe that there may be administrative errors in year of birth or address number, we may want to construct a measure of similarity by simply taking the absolute value of the difference between the observations for each pair. The closer the difference is to 0, the more likely they are to be a match on that variable. We can make that comparison using the \texttt{numeric.match} argument in \texttt{fastLink()}, as follows:
...
Of the six variables we specified to match on, three (\texttt{firstname}, \texttt{lastname}, and \texttt{streetname}) are now being compared using Jaro-Winkler string distances as described above, and one (\texttt{birthyear}) is being compared with numeric distances. The final two variables, \texttt{city} and \texttt{housenum}, use exact comparisons (the default). We have also specified the threshold below which a numeric comparison is declared a match using \texttt{cut.a.num} --- here, if the birth year of observation $i$ and observation $j$ are less than $\pm 1$ year apart or less, it is declared a match.

## Incorporating Partial Match Categories{#subsec:partialmatch}

So far, we have introduced string distance and numeric distance comparisons in terms of a single cutoff --- if the comparison value is above the cutoff, it is declared a match, while if it is below the cutoff, it is declared a non-match. For both comparison types, though, we can introduce partial matching categories that can take advantage of more information in the data. Here, a comparison is declared a match if it is above the threshold provided in \texttt{cut.a} for a string distance comparison and \texttt{cut.a.num} for a numeric distance comparison, and it will be considered a partial match if its value is above the threshold given in \texttt{cut.p} (\texttt{cut.p.num}) and below the threshold given in \texttt{cut.a} (\texttt{cut.a.num}) for string.distance comparisons (numeric distance comparisons). If the value of the comparison is below \texttt{cut.p} for string distance comparisons or \texttt{cut.p.num} for numeric comparisons, it is declared a non-match. We can specify partial match comparisons as follows, using the argument \texttt{partial.match}:
...
As before, we are matching the variables \texttt{firstname}, \texttt{lastname}, and \texttt{streetname} using string-distance matching, \texttt{birthyear} using numeric distance matching, and \texttt{housenum} and \texttt{city} using exact matching. However, we've also specified that the comparisons for \texttt{streetname} and \texttt{birthyear} include a partial match category, and we've also noted the lower cutoffs for declaring a comparison a partial match using the arguments \texttt{cut.p} and \texttt{cut.p.num}. For all other variables without a partial match category, the cutoff for match/non-match is still \texttt{cut.a} and \texttt{cut.a.num}.
...
We demonstrate a simple example of this workflow below. We start by creating two small random samples of our test data, \texttt{dfA.s} and \texttt{dfB.s}, by randomly sampling 30\% of of the observations in each. Since we're only estimating the model on this subset and not actually getting matched pairs, we can tell \texttt{fastLink()} to only return the EM object by specifying \texttt{estimate.only = TRUE}. This not only reduces the computational time for estimating and returning the \fastLink \ object, but it also returns the EM object in a form that can be fed back into \texttt{fastLink()} when getting posterior match probabilities for the full sample (\texttt{fl\_out\_rs}).
...
Finally, we take the estimated model parameters and feed them back into \texttt{fastLink()} to predict posterior match probabilities on the full data set. The EM object is simply fed back into \texttt{fastLink()} using the \texttt{em.obj} argument, which tells the function to take the estimated match parameters and apply them to the match patterns in the full data set. The user can also split up the full data set and provide the same estimated model parameters to each separate run of \texttt{fastLink()} to further reduce the computational burden.
...
We implement the log-linear model in \fastLink \ through the option \texttt{cond.indep} --- when set to true (default), \texttt{fastLink()} runs the standard Fellegi-Sunter conditionally independent model. When set to false, it substitutes the log-linear modeling strategy in the EM algorithm to model dependencies between linkage fields. 
...
\fastLink \ can also be used to de-duplicate a single data frame using the PRL framework. Within-data-frame dedupliation is straightforward in \fastLink --- the user simply provides the same data frame to both \texttt{dfA} and \texttt{dfB} arguments, and runs \texttt{fastLink()} as they normally would. The \texttt{blockData()} function then takes both data frames and the resulting output from \texttt{fastLink()}, and returns a final data frame with a new ID column indicating identical observations.

Below is a simple workflow to de-duplicate \texttt{dfA}, after adding 10 duplicated observations manually:
...
Users can then dedupe data sets using the \texttt{dedupe.ids} covariate in the returned data frame.
...
We can then feed these into \texttt{fastLink()} using the \texttt{priors.obj} argument. In order to properly specify the prior, we also need to tell \texttt{fastLink()} how much to weight the prior estimates relative to the parameter estimates implied by the observed match patterns. To weight the priors, we specify the arguments \texttt{w.lambda} and \texttt{w.pi} as the weights for the $\lambda^{\rm prior}$ and $\pi_{{\rm adr}, 1, 0}^{\rm prior}$, respectively. Specifying \texttt{w.lambda = .25} tells \texttt{fastLink()} that the final estimate for $\lambda$ should be a weighted average where 25\% is the prior estimate, and 75\% is the maximum likelihood estimate from the observed match matterns. Lastly, when specifying a prior on the address field, we tell \texttt{fastLink()} which field is an address field using the \texttt{address.field} argument. An example, with priors calibrated for the test data set, is:
...
We place Beta priors on the relevant parameters to maintain conjugacy, while leaving the priors on the remaining parameters improper. Other data sources for auxiliary information on migration can be used in \texttt{fastLink()} by feeding them in as a list with the names \texttt{lambda.prior} or \texttt{pi.prior} as above. 
...
To conduct an ex-post adjustment on observed name frequencies, the user simply sets \texttt{reweight.names = TRUE} in \texttt{fastLink()} and provides the name of the first name field in \texttt{firstname.field}, as follows:
...
Below we show a simple example where we use \textsf{probablepeople} to parse strings of corporation names and personal names into separate components. For a given string, we simply use the \texttt{tag()} function in the \textsf{probablepeople} library, which outputs an ordered dictionary where each identified component is assigned to one of several pre-defined tags. For names, these tags include \texttt{FirstName}, \texttt{Surname}, \texttt{Nickname}, \texttt{PrefixMarital}, and others.\footnote{See the documentation for \textsf{probablepeople} at (https://probablepeople.readthedocs.io/en/latest/) for more detail.} Corporations have their own set of pre-defined tags for the separate components.
...
We can apply the same methodology to address strings in order to parse them into usable components. Again, the \texttt{tag()} function will take an input string and output an ordered dictionary with pre-defined tags. Valid tags for addresses include \texttt{AddressNumber}, \texttt{BuildingName}, \texttt{StateName}, \texttt{PlaceName}, and \texttt{ZipCode}, among other tags.\footnote{See the documentation for \textsf{usaddress} at (https://usaddress.readthedocs.io/en/latest/) for more detail.}
...
The function \textsf{preprocText()} takes as argument a vector of text, which it assumes has already been split into fields, and then provides a number of options for conversion. The options applicable to all string types include \texttt{tolower} (convert all text to lower-case), \texttt{remove\_whitespace} (strip leading and trailing whitespace, and convert multiple spaces to single space), and \texttt{remove\_punctuation} (remove all punctuation such as apostrophes, semi-colons, colons, periods, question marks, etc.). For example, we can convert text as follows:
...
For names, \texttt{preprocText()} also implements Soundex encoding from the \texttt{stringdist} package. The Soundex algorithm is used by the U.S. Census Bureau to increase the likelihood of matches across similar-sounding names --- for example, while Rupert and Robert are spelled differently and would be coded as non-matches by any string distance measure, Soundex encodings of both names are identical. We refer users to other references (e.g. @knut:72, pgs. 391-92) for a full description of the algorithm, but as an overview, the algorithm keeps the first letter of a string and then follows a series of rules to convert the remaining portion of the string to a three-digit numerical code based on letter combinations that are similar in sound. Below, we show the output from a Soundex conversion of Rupert and Robert:

```{r preproc_soundex, results = 'markup', echo = TRUE, tidy = FALSE, fig.pos = "t!"}
preprocText(text = c("Rupert", "Robert"), soundex = TRUE)
```

Finally, for addresses, we also implement United States Postal Service (USPS) address standardization of street names to ensure all street types (for example, Road, Street, and Avenue) are consistently recorded across data sets. The USPS converts all street types to standard abbreviations (such as `Road' to `Rd.' and `Street' to `St.'), and we implement the same conversion in \texttt{preprocText()} through the \texttt{usps\_address} argument:
...
In \fastLink, the function \texttt{blockData()} can block two data sets using a single variable or combinations of variables using several different blocking techniques. The basic functionality is similar to that of \texttt{fastLink()}, where the analyst inputs two data sets and a vector of variable names that they want to block on. A simple example follows, where we are blocking the two sample data sets by gender:
...
Furthermore, \texttt{blockData()} allows analysts to go beyond exact blocking on a single variable. Users can block on multiple variables by feeding a vector of variable names to \texttt{varnames}, as follows:
...
In addition to the blocking functionalities, \fastLink\ also includes methods to help researchers discard observations with no obvious matching candidates in the paired data set using string distance comparisons. \texttt{stringSubset()} calculates the string distance between each pair of observations in two data sets and returns the indices where there is at least one candidate match with a string similarity measure above a certain threshold. That is, it reduces the set of candidate matches by throwing out any observation in dataset A where no observation in dataset B has a sufficiently similar first name/last name/street name, and vice versa. 
...
In the output object, \texttt{dfA.inds} contains the indices of the observations in \texttt{dfA} where at least one observation in \texttt{dfB} has a sufficiently similar first name, and vice versa for \texttt{dfB.inds}. This procedure has also reduced the number of possible comparisons  --- whereas the standard merge has to make `r nrow(dfA) * nrow(dfB)` comparisons, now \texttt{fastLink()} only has to make `r length(stringsub_out$dfA.inds) * length(stringsub_out$dfB.inds)` comparisons within the subsetted data frames. \texttt{stringSubset()} also allows users to specify Jaro string-distance measures and Levenshtein distances instead of the default Jaro-Winkler distance.
