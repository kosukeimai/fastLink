...
tableCounts <- function(g1p = x, g2p = NULL, g3p = NULL, g4p = NULL, g5p = NULL, g6p = NULL, nr1 = y, nr2 = z) {
    ...
    m.func <- function(x) {
	  ...
	  m.5 <- add.nas(m.5, name.5[[1]], name.5[[2]], 2^15)
   	...
   	rm(name.na, name.1, name.2, name.3, name.4, name.5, name.6)
		...
		requireNamespace('data.table')
   	    ...
   	    rm(T)
   	    M <- as.matrix(M)
		counts <- rbind(M, c(0, (as.numeric(nrow(m.f)) * as.numeric(ncol(m.f)) - sum(M[, 2]))))
		rm(M, m.f)
		return(counts)
   	}
  	...
  	gammas <- foreach(do = 1:(nrow(ind))) %dopar% {
			...
			i <- ind[do, 1]; j <- ind[do, 2]
  			...
  			step1 <- list(temp, temp.c, tempna, lims)
  			rm(temp, temp.c, tempna, lims)
  	...
  	rm(temp.1, temp.2, temp.3, temp.4, temp.5, temp.6); gc()
  	rm(temp.1.c, temp.2.c, temp.3.c, temp.4.c, temp.5.c, temp.6.c); gc()
  	rm(temp.1na, temp.2na, temp.3na, temp.4na, temp.5na, temp.6na); gc()
