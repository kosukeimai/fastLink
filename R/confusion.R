...
confusion <- function(object, threshold = .85) {
    ...
    A <- sum(fL$posterior * ifelse(fL$posterior >= threshold, 1, 0))
    B <- sum((1 - fL$posterior) * ifelse(fL$posterior >= threshold, 1, 0))
    C <- sum(fL$posterior * ifelse(fL$posterior < threshold, 1, 0)) + (min(fL$nobs.a, fL$nobs.b) - A) * 0.001
    D <- sum((1 - fL$posterior) * ifelse(fL$posterior < threshold, 1, 0)) + (min(fL$nobs.a, fL$nobs.b) - A) * (1 - 0.001)
