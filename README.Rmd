...
have six commonly named fields:
...
For instance, if we wanted to include partial matches on `firstname`, `lastname`, and `address`, but only do exact matches on `city` and `middlename`, we would
run:
