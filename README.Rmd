...
If the provided variances are too large (forcing the parameter values for the $$\gamma$$ prior or the $$\pi_{k,l}$$ prior below 1), the function will choose new parameter values by testing the sequence $$1/(10^i)$$ until those restrictions are satisfied. The means and variances used to calculate optimal paramter values can be viewed in the `parameter_values` field of the output.
