...
matchesLink <- function(gammalist, nr1 = w, nr2 = p, em = y, cut = z) {
    ...
    temp.1 <- gammalist[[1]]$matches2
    temp.2 <- gammalist[[2]]$matches2
    temp.3 <- gammalist[[3]]$matches2
    temp.4 <- gammalist[[4]]$matches2
    temp.5 <- gammalist[[5]]$matches2
    temp.6 <- gammalist[[6]]$matches2

    ## Indices for somehow close strings:
    temp.1.c <- gammalist[[1]]$matches1
    temp.2.c <- gammalist[[2]]$matches1
    temp.3.c <- gammalist[[3]]$matches1
    temp.4.c <- gammalist[[4]]$matches1
    temp.5.c <- gammalist[[5]]$matches1
    temp.6.c <- gammalist[[6]]$matches1

    ## Indexes for NAs:
    temp.1na <- gammalist[[1]]$nas
    temp.2na <- gammalist[[2]]$nas
    temp.3na <- gammalist[[3]]$nas
    temp.4na <- gammalist[[4]]$nas
    temp.5na <- gammalist[[5]]$nas
    temp.6na <- gammalist[[6]]$nas
